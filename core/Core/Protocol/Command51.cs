...
{
    internal sealed class Command51
        : Command, IRequestCommand
    {
        ...
        public Command51(CommandBuilder command)
        {
            if (command == null)
        ...
        public Command51(byte[] commandID, ushort reserved, ushort hopCount, byte[] senderPeerID, byte[] sourceSearchID, byte[] hashedFileHash)
        {
            if (commandID == null)
            ...
            m_Command = new CommandBuilder(CommandBuilder.EncryptionMethod.Rijndael, 0x51);
            m_Command.Write(0x51);
