...
{
    internal sealed class Command41
        : Command, IResponseCommand
    {
        ...
        public Command41(CommandBuilder command)
        {
            if (command == null)
        ...
        public Command41(byte[] commandID, byte[] senderPeerID, byte[] receiverPeerID, byte[] downloadID, uint queuePosition)
        {
            if (commandID == null)
            ...
            m_Command = new CommandBuilder(CommandBuilder.EncryptionMethod.Rijndael, 0x41);
            m_Command.Write((byte)0x41);
