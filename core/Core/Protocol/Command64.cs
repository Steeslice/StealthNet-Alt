...
{
    internal sealed class Command64
        : Command, IResponseCommand
    {
        ...
        public Command64(byte[] commandID, byte[] senderPeerID, byte[] receiverPeerID, byte[] sourceSearchID, byte[] sectorsMap)
        {
            if (commandID == null)
            ...
            m_Command = new CommandBuilder(CommandBuilder.EncryptionMethod.Rijndael, 0x64);
            m_Command.Write(0x64);
