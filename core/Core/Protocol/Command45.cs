...
{
    internal sealed class Command45
        : Command, IResponseCommand
    {
        ...
        public Command45(CommandBuilder command)
        {
            if (command == null)
        ...
        public Command45(byte[] commandID, byte[] senderPeerID, byte[] receiverPeerID, byte[] downloadID, uint sector)
        {
            if (commandID == null)
            ...
            m_Sector = sector;
            m_Command = new CommandBuilder(CommandBuilder.EncryptionMethod.Rijndael, 0x45);
            m_Command.Write((byte)0x45);
