...
{
    internal sealed class Command74
        : Command, IResponseCommand
    {
        ...
        public Command74(CommandBuilder command)
        {
            if (command == null)
        ...
        public Command74(byte[] commandID, byte[] senderPeerID, byte[] receiverPeerID, byte[] downloadID, byte[] hashedFileHash)
        {
            if (commandID == null)
            ...
            m_Command = new CommandBuilder(CommandBuilder.EncryptionMethod.Rijndael, 0x74);
            m_Command.Write((byte)0x74);
