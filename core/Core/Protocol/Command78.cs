...
{
    internal sealed class Command78
        : Command, IResponseCommand
    {
        ...
        public Command78(CommandBuilder command)
        {
            if (command == null)
        ...
        public Command78(byte[] commandID, byte[] senderPeerID, byte[] receiverPeerID, byte[] downloadID, uint sector)
        {
            if (commandID == null)
            ...
            m_Sector = sector;
            m_Command = new CommandBuilder(CommandBuilder.EncryptionMethod.Rijndael, 0x78);
            m_Command.Write((byte)0x78);
