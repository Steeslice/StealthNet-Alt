...
{
    internal sealed class Command76
        : Command, IResponseCommand
    {
        ...
        public Command76(CommandBuilder command)
        {
            if (command == null)
        ...
        public Command76(byte[] commandID, byte[] senderPeerID, byte[] receiverPeerID, byte[] downloadID)
        {
            if (commandID == null)
            ...
            m_Command = new CommandBuilder(CommandBuilder.EncryptionMethod.Rijndael, 0x76);
            m_Command.Write((byte)0x76);
