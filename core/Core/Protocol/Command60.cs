...
{
    internal sealed class Command60
        : Command, IRequestCommand
    {
        ...
        public Command60(CommandBuilder command)
        {
            if (command == null)
        ...
        public Command60(byte[] commandID, byte[] floodingHash, byte[] senderPeerID, byte[] sourceSearchID, byte[] hashedFileHash)
        {
            if (commandID == null)
            ...
            m_Command = new CommandBuilder(CommandBuilder.EncryptionMethod.Rijndael, 0x60);
            m_Command.Write(0x60);
