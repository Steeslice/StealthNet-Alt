...
{
    internal sealed class Command20
        : Command, IRequestCommand
    {
        ...
        public Command20(CommandBuilder command)
        {
            if (command == null)
        ...
        public Command20(byte[] commandID, byte[] floodingHash, byte[] senderPeerID, byte[] searchID, string searchPattern)
        {
            if (commandID == null)
            ...
            m_Command = new CommandBuilder(CommandBuilder.EncryptionMethod.Rijndael, 0x20);
            m_Command.Write(0x20);
