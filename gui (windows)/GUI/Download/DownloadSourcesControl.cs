...
{
    internal sealed partial class DownloadSourcesControl
        : UserControl
    {
        ...
        private void updateTimer_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    m_Download.Sources.Lock();
                    {
                        DataGridViewRow row;
                        for (int n = sourcesDataGridView.Rows.Count - 1; n >= 0; n--)
                        {
                            row = sourcesDataGridView.Rows[n];
                            if (!m_Download.Sources.ContainsKey(((RandomTag<string>)row.Tag).Tag))
                    ...
                    int a = 0;
                    foreach (Download.Source source in m_Download.Sources.Values)
                    {
                        if (a % 50 == 0)
                        ...
                        // T.Norad: BZ 123: display no info if the source has no information about the filename
                        if (!source.HasInformation)
                        {
                            sourceRow.Cells["FileName"].Value = Properties.Resources.ProgressBar_NoInfo;
                        }
                        else
                        {
                ...
                {
                    Logger.Instance.Log(ex, "An exception was thrown! (DownloadResultSourcesControl)");
                }
                finally
                {
                    m_Download.Sources.Unlock();
                }
            }
            finally
            {
